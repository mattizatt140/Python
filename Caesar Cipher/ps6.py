...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letter in string.ascii_letters:
            shiftDict[letter] = None
        for i in range(26):
            try:
                shiftDict[lowercase[i]] = lowercase[i + shift]
            except IndexError:
                shiftDict[lowercase[i]] = lowercase[i + shift - 26]
        for i in range(26):
            try:
                shiftDict[uppercase[i]] = uppercase[i + shift]
            except IndexError:
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            goodCount = 0
            copyMessage = Message.apply_shift(self, i).lower()
            LoW = copyMessage.split(' ')
            for word in LoW:
                if is_word(self.valid_words, word):
                    goodCount += 1
            if goodCount > bestShiftCount:
                bestShift = i
                bestShiftCount = goodCount
        self.message_text = Message.apply_shift(self, bestShift)
