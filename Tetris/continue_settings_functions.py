...
def check_click_events(event, settings, stats, title, buttons, texts, text_input, sounds, x, y):
    if buttons.get("main_menu_button").rect.collidepoint(x, y):
        ...
        stats.continue_menu_active, stats.continue_settings_active = True, False
        ...
        buttons.recolor(settings)
        
        texts.recolor(settings)
...
def update_screen(settings, screen, stats, menu, buttons, texts):
    menu.draw_outline()
