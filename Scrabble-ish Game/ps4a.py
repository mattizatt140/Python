...
WORDLIST_FILENAME = "C:/D/Python/Scrabble-ish Game/words.txt"
...
def loadWords():
    ...
    wordList = {}
    for line in inFile:
        wordList[(line.strip().lower())] = getWordScore((line.strip().lower()), HAND_SIZE)
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList.keys():
        for i in range(len(word)):
            if word[i] in copyHand.keys() and copyHand[word[i]] > 0:
                copyHand[word[i]] -= 1
                if i == len(word) - 1:
                    return True
            else:
...
def calculateHandlen(hand):
    ...
    """
    counter = 0
    for i in hand.keys(): counter += hand[i]
    return counter
        ...
        # Otherwise (the input is not a single period):
            ...
            # If the word is not valid:
                ...
                # Reject invalid word (print a message followed by a blank line)
                    ...
                    totalScore += wordList[userInput]
                    print('"' + userInput + '"', "earned", getWordScore(userInput, n), "points. Total:", totalScore, "points."); print("")
                  
            # Otherwise (the word is valid):
                else:
                    print("Invalid word, please try again."); print("")
